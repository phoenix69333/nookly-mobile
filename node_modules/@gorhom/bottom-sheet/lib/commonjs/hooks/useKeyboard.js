...
const useKeyboard = () => {
  ...
  //#region worklets
  const handleKeyboardEvent = (0, _react.useCallback)((state, height, duration, easing, bottomOffset) => {
    'worklet';
  ...
  (0, _react.useEffect)(() => {
    const handleOnKeyboardShow = event => {
      (0, _reactNativeReanimated.runOnUI)(handleKeyboardEvent)(_constants.KEYBOARD_STATE.SHOWN, event.endCoordinates.height, event.duration, event.easing, _constants.SCREEN_HEIGHT - event.endCoordinates.height - event.endCoordinates.screenY);
    };
